...
:root{
  ...
  --good:#2f855a; --fair:#b7791f; --poor:#c53030;
  --bg:var(--cirrus); --surface:#fff; --edge:rgba(0,0,0,.08);
  --text:var(--ink); --muted:var(--slate); --accent:var(--orbit);
  --sans:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;
}
@media (prefers-color-scheme:dark){
  :root{--bg:var(--ink);--surface:var(--dusk);--edge:rgba(255,255,255,.07);
        --text:var(--starlight);--muted:var(--haze);--accent:var(--stellar);}
}
*{box-sizing:border-box}
body{margin:0;background:var(--bg);color:var(--text);font-family:var(--sans);
     font-size:15px;line-height:1.65;letter-spacing:.01em;padding:40px 20px 80px;
     -webkit-font-smoothing:antialiased}
.wrap{max-width:880px;margin:0 auto}
h1{font-size:28px;font-weight:400;margin:0 0 4px;letter-spacing:-.01em}
...
def build(results, field=None, findings_by_url=None, generated=''):
    ...
    out.append('<footer>Generated by <a href="https://considus.com/pagespeed-insights-mcp/">'
               'PageSpeed Insights MCP</a>, which reports the median of several distinct '
               'analyses with its spread, because a single measurement of a noisy '
               'instrument is not a measurement. Unofficial, not affiliated with Google '
               'LLC.</footer>')
    return ('<!doctype html><html lang="en"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width,initial-scale=1">'
            f'<title>Page speed report</title><style>{CSS}</style></head>'
            f'<body><div class="wrap">{"".join(out)}</div></body></html>')
